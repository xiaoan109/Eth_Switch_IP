/*
    ...
    Filename:      S55NLLGDPH_X512Y8D12_BW.v
    IP code :      S55NLLGDPH
    Version:       1.3.a
...
module S55NLLGDPH_X512Y8D12_BW (
                         QA,
                         QB,
			  CLKA,
			  CLKB,
...
task rw_memB;
  ...
  specify
    ...
    $period(posedge CLKA,3.740,CLKA_CYC_flag);
    $width(posedge CLKA,1.122,0,CLKA_H_flag);
    $width(negedge CLKA,1.122,0,CLKA_L_flag);
    
    $setuphold(posedge CLKB,posedge CENB,1.000,0.500,CENB_flag);
    $setuphold(posedge CLKB,negedge CENB,1.000,0.500,CENB_flag);
    $period(posedge CLKB,3.740,CLKB_CYC_flag);
    $width(posedge CLKB,1.122,0,CLKB_H_flag);
    $width(negedge CLKB,1.122,0,CLKB_L_flag);
